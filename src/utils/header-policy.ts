 ...
 * - `dist/_headers`：Cloudflare Pages / Netlify 读**发布目录**里的这个文件，由 `scripts/build-headers.ts` 生成；
 * - `vercel.ts`：Vercel 只认仓根的配置文件，但它支持 TypeScript——那份**在 Vercel 构建时执行**，
 *   直接读环境变量，所以仓库里不需要提交任何生成物。
 *
 * 两边都必须知道外部来源，而它随 env 变化：配了 remark42（`PUBLIC_REMARK42_HOST`）就把
 * 那个域名加进白名单——静态文件自己没办法知道这件事，所以 CF 侧必须生成而不是手写。
 * 目前策略是 Report-Only（只上报不拦），观察期结束、白名单补齐后再去掉 -Report-Only 强制。
 */
...
const HEADERS_DOC = [
  ...
  "# ⚠️ 本文件由 `node scripts/build-headers.ts` 生成，改这里会被下次构建覆盖——",
  "#    要改策略请改 src/utils/header-policy.ts。",
...
function toVercelSource(source: string): string {
